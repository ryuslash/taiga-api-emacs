...
(describe "Reading a task from an alist"
  (with-read-data (task "task")
    (with-subject task
      (it "yields a task" (expect (taiga-api-task-p task) :to-be-truthy))
      (it "includes assigned-to" (expect (its 'assigned-to) :to-be 19))
      (it "includes blocked-note" (expect (its 'blocked-note) :to-equal ""))
      (it "includes blocked-note-html" (expect (its 'blocked-note-html) :to-equal ""))
      (it "includes comment" (expect (its 'comment) :to-equal ""))
      (it "includes milestone-slug" (expect (its 'milestone-slug) :to-equal "sprint-2014-11-1"))
      (it "includes created-date" (expect (its 'created-date) :to-equal "2014-07-29T07:15:50+0000"))
      (it "includes description" (expect (its 'description) :to-equal "Implement API CALL"))
      (it "includes description-html" (expect (its 'description-html) :to-equal "<p>Implement API CALL</p>"))
      (it "includes id" (expect (its 'id) :to-be 1149))
      (it "includes is-blocked" (expect (its 'is-blocked) :not :to-be-truthy))
      (it "includes is-closed" (expect (its 'is-closed) :to-be-truthy))
      (it "includes milestone" (expect (its 'milestone) :to-be 98))
      (it "includes modified-date" (expect (its 'modified-date) :to-equal "2014-10-10T12:07:13+0000"))
      (it "includes finished-date" (expect (its 'finished-date) :to-be nil))
      (it "includes owner" (expect (its 'owner) :to-be 19))
      (it "includes project" (expect (its 'project) :to-be 31))
      (it "includes user-story" (expect (its 'user-story) :to-be 17))
      (it "includes ref" (expect (its 'ref) :to-be 31))
      (it "includes status" (expect (its 'status) :to-be 83))
      (it "includes subject" (expect (its 'subject) :to-equal "Customer personal data"))
      (it "includes tags" (expect (its 'tags) :to-equal ["service catalog" "customer"]))
      (it "includes us-order" (expect (its 'us-order) :to-be 1))
      (it "includes taskboard-order" (expect (its 'taskboard-order) :to-be 1))
      (it "includes version" (expect (its 'version) :to-be 8))
      (it "includes is-iocaine" (expect (its 'is-iocaine) :not :to-be-truthy))
      (it "includes external-reference" (expect (its 'external-reference) :to-be nil))
      (it "includes watchers" (expect (arrayp (its 'watchers)) :to-be-truthy))
      (it "includes neighbors" (expect (taiga-api-neighbors-p (its 'neighbors)) :to-be-truthy)))))
